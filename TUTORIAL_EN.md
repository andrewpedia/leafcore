...
* Multi-core support. Leaf utilize its modules and [leaf/go](https://leafcore/tree/master/go) to make use of CPU resouces at maximum while avoiding varieties of side effects may be caused.
...
A game server implemented with Leaf may include many modules (e.g. [LeafServer](https://leafcoreserver)) which all share below traits:

* Each module runs inside a separate goroutine
* Modules communicate with one another via a light weight RPC channel([leaf/chanrpc](https://leafcore/tree/master/chanrpc))
...
[LeafServer](https://leafcoreserver) is a game server developped with Leaf. Let's start with it.

Download the source code of LeafServer：

```
git clone https://leafcoreserver
```
...
More references are at [leaf/go](https://leafcore/blob/master/go)。
...
More references are at [leaf/timer](https://leafcore/blob/master/timer)。
...
More references are at [leaf/log](https://leafcore/blob/master/log).

### Leaf recordfile

Leaf recordfile is formatted in CSV([Example](https://leafcore/blob/master/recordfile/test.txt)). recordfile is to manage the configuration for game. The usage of recordfile in LeafServer is quite simple:
...
Refer to [leaf/recordfile](https://leafcore/blob/master/recordfile) for more details.

Learn more
----------

More references are at Wiki [https://leafcore/wiki](https://leafcore/wiki)
